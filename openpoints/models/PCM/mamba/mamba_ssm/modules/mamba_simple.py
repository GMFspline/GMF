    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v2":
                ...
                cxz = xz.reshape(batch,-1,8,8)
                cxz = cxz.permute(0,1,3,2)
                cxz = cxz.reshape(batch,-1,64)
                ...
                out = F.linear(rearrange(out + out_b.flip([-1])+out_c + out_d.flip([-1]), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
            else:
                out = mamba_inner_fn(
                    xz,
                    self.conv1d.weight,
